...
"""selectors for this spider in scrapy shell
title = response.css('h3.wd-entities-title a::text').get()
image = response.css('div.wd-product-wrapper img.attachment-large::attr(data-src)').get()
price = response.css('div.wrap-price ins span.woocommerce-Price-amount bdi::text').get()
!also price = response.css('span.screen-reader-text:contains("Current price is")::text').get()
url = response.css("div.wd-product-thumb.product-element-top.wd-quick-shop a::attr(href)").get()
vendor = "https://hardwaremarket.net"
"""
